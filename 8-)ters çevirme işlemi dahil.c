...
void tersCevir(){
    ...
    temp=start;
    while(temp!=NULL){
        temp2=temp->next;// tempin nextini temp 2 yaptık yani sonraki elemanı
        temp->next=prev; // tempin nextine bi düğüm inşa ettim
        prev=temp; // inşa ettiğim düğümün değerlerini filan tempe başlangıç değere eşitledim
        temp=temp2; // oluşturmuş olduğum ilk düğümün değerlerinide sonraki değerleriin içine attım
